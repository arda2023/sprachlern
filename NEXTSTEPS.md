...
- „Hinzufügen": Spinner statt Link bei gleicher Breite, Feld/Umschalter gesperrt, Sperre vor dem ersten `await` → kein Doppelspeichern. Bei Fehler Text in `--error` unter dem Feld, Eingabe bleibt für Retry. Zustände in design.md 6 ergänzt.
...
- Ob die Function mit Login-Token aufgerufen werden darf (JWT-Prüfung) und ob `gemini-3.8-flash` antwortet. Ein Fehler zeigt nur den allgemeinen Hinweis; die Ursache steht nicht im UI.
## Offene Probleme
- Viele Einträge = viele Aufrufe nacheinander; ohne Fortschrittsanzeige wirkt ein großer Batch lange. Ladefehler beim Öffnen des Custom-Stapels erscheinen weiter nur im Log.
